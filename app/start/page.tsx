import { FunnelHeader } from "@/components/funnel/Brand";
import { FunnelProgress } from "@/components/funnel/FunnelProgress";
import { StartForm } from "./StartForm";

export default function StartPage() {
  return (
    <div className="flex min-h-screen flex-col bg-cream-1">
      <FunnelHeader />
      <main className="mx-auto w-full max-w-2xl flex-1 px-6 py-12 md:px-10 md:py-20">
        <FunnelProgress current={1} />
        <h1 className="h2 mt-6 text-ink">
          Where is the work getting <span className="ital">stuck</span>?
        </h1>
        <p className="mt-6 text-lg leading-relaxed text-ink-2">
          Send me your website and the bottleneck eating your week. I&apos;ll
          read the site, sketch three concrete build ideas, and help you pick
          the first thing worth turning into software.
        </p>
        <div className="mt-10">
          <StartForm />
        </div>
      </main>
    </div>
  );
}
